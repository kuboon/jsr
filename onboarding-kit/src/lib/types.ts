 ...
 *
 * `anchor()` only ever flips to the *opposite* placement when a tooltip would overflow — it does
 * not cascade through the other sides — so treat this as a preference that is honored or mirrored,
 * and set it explicitly on steps that must appear beside their target rather than above or below
 * it.
 */
 ...
 *
 * Use this for steps whose subject is not a single element — a region of a canvas, a chart area,
 * the space where something will appear.
 */
 ...
 *
 * A selector written in JSON is decoupled from the component tree, which is the point — but it
 * also means a step can name an element that is not on this page, is inside a collapsed section,
 * or has not hydrated yet. Every step therefore has a policy, and the default is to move on rather
 * than dead-end the tour.
 *
