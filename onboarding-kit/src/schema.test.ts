...
describe("tourSchema", () => {
  it("is published with the $id an editor resolves", () => {
    expect(tourSchema.$id).toBe(
      "https://jsr.io/@kuboon/onboarding-kit/schema.json",
    );
  });
});
...
describe("parseScenario", () => {
  it("accepts a minimal scenario", () => {
    const scenario = parseScenario({ name: "tour", steps: [{ body: "hi" }] });
  ...
  it("keeps every optional field it recognizes", () => {
    const scenario = parseScenario({
      ...
      steps: [
        ...
        {
          target: { x: 10, y: 20, width: 30, height: 40 },
          body: "b",
          spotlight: false,
        },
      ],
    ...
    expect(scenario.steps[1].target).toEqual({
      x: 10,
      y: 20,
      width: 30,
      height: 40,
  ...
  it("names the offending field", () => {
    ...
    try {
      parseScenario({
        name: "tour",
        steps: [{ body: "ok" }, { body: "x", placement: "sideways" }],
      });
    } catch (thrown) {
      error = thrown;
    }
  ...
  it("rejects a scenario with no name or no steps", () => {
    expect(() => parseScenario({ steps: [{ body: "x" }] })).toThrow(
      TourScenarioError,
    );
    expect(() => parseScenario({ name: "tour", steps: [] })).toThrow(
      TourScenarioError,
    );
  });

  it("rejects a step with nothing to say", () => {
    expect(() => parseScenario({ name: "tour", steps: [{ target: "#a" }] }))
      .toThrow(
        TourScenarioError,
      );
  });
  ...
  it("rejects a point target with no coordinates", () => {
    expect(() =>
      parseScenario({ name: "t", steps: [{ body: "x", target: { x: 1 } }] })
    ).toThrow(
      /target\.y/,
    );
  });
