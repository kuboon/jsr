...
const WHEN_MISSING: readonly TourWhenMissing[] = [
  "skip",
  "wait",
  "center",
  "fail",
 ...
 *
 * @param value Parsed JSON, from a fetch, a `<script type="application/json">`, or a property
...
export function parseScenario(value: unknown): TourScenario {
  ...
  if (root.autoStart !== undefined) {
    scenario.autoStart = expectBoolean(root.autoStart, "autoStart");
  }
  if (root.keyboard !== undefined) {
    scenario.keyboard = expectBoolean(root.keyboard, "keyboard");
  }
  if (root.labels !== undefined) scenario.labels = parseLabels(root.labels);
  if (root.defaults !== undefined) {
    scenario.defaults = parseStepDefaults(
      expectObject(root.defaults, "defaults"),
      "defaults",
    );
  }
...
function parseLabels(value: unknown): TourLabels {
  const raw = expectObject(value, "labels");
  const labels: TourLabels = {};
  for (const key of ["next", "back", "skip", "done", "progress"] as const) {
    if (raw[key] !== undefined) {
      labels[key] = expectString(raw[key], `labels.${key}`);
    }
  }
  return labels;
}

function parseStepDefaults(
  raw: Record<string, unknown>,
  path: string,
): TourStepDefaults {
  const out: TourStepDefaults = {};
  if (raw.placement !== undefined) {
    out.placement = expectEnum(raw.placement, PLACEMENTS, `${path}.placement`);
  }
  if (raw.offset !== undefined) {
    out.offset = expectNumber(raw.offset, `${path}.offset`);
  }
  if (raw.spotlight !== undefined) {
    out.spotlight = expectBoolean(raw.spotlight, `${path}.spotlight`);
  }
  if (raw.spotlightPadding !== undefined) {
    out.spotlightPadding = expectNumber(
      raw.spotlightPadding,
      `${path}.spotlightPadding`,
    );
  }
  if (raw.scrollIntoView !== undefined) {
    out.scrollIntoView = expectBoolean(
      raw.scrollIntoView,
      `${path}.scrollIntoView`,
    );
  }
  if (raw.whenMissing !== undefined) {
    out.whenMissing = expectEnum(
      raw.whenMissing,
      WHEN_MISSING,
      `${path}.whenMissing`,
    );
...
function parseStep(value: unknown, path: string): TourStep {
  ...
  const step: TourStep = parseStepDefaults(raw, path);

  if (raw.id !== undefined) step.id = expectString(raw.id, `${path}.id`);
  if (raw.title !== undefined) {
    step.title = expectString(raw.title, `${path}.title`);
  }
  if (raw.body !== undefined) {
    step.body = expectString(raw.body, `${path}.body`);
  }
  if (raw.target !== undefined) {
    step.target = parseTarget(raw.target, `${path}.target`);
  }

  if (step.title === undefined && step.body === undefined) {
    throw new TourScenarioError(
      path,
      'expected at least one of "title" or "body"',
    );
  }
...
function parseTarget(value: unknown, path: string): string | TourPoint {
  if (typeof value === "string") {
    if (value === "") {
      throw new TourScenarioError(path, "expected a non-empty selector");
    }
    return value;
  }
  ...
  if (raw.width !== undefined) {
    point.width = expectNumber(raw.width, `${path}.width`);
  }
  if (raw.height !== undefined) {
    point.height = expectNumber(raw.height, `${path}.height`);
  }
  return point;
}
...
function expectString(value: unknown, path: string): string {
  if (typeof value !== "string") {
    throw new TourScenarioError(path, "expected a string");
  }
  return value;
}
...
function expectBoolean(value: unknown, path: string): boolean {
  if (typeof value !== "boolean") {
    throw new TourScenarioError(path, "expected a boolean");
  }
  return value;
}
